...
class find_all_data(APIView):
    def post(self, request):
        date = request.data.get('date')
        data = all_data.objects.filter(date=date).values_list('date', 'title', 'author', 'type', 'content', 'entities')
        print(data)
        json_list = []
        for i in data:
            json_list.append({
...
'''
    标题获取所有数据
    url: http://127.0.0.1:8000/title_find_data
    请求形式：
    {
...
class title_find_data(APIView):
    def post(self, request):
        title = request.data.get('title')
        data = all_data.objects.filter(title=title)
        json_list = []
        for i in data:
            json_list.append({
                'date': i.date,
...
'''
    标题返回10条相似的标题
    url: http://127.0.0.1:8000/find_similar_title
    请求形式：
    {
        "title": "China reopened wildlife markets."
    }
...
class find_similar_title(APIView):
    def post(self, request):
        target_title = request.data.get('title')
        findTitles = findAllTitle()
        title_data = all_data.objects.all().values_list('title')
        title_list = []
        for i in title_data:
